...
$_SESSION['coor_xb'] = array('ENE','FEB','MAR','ABR','MAY','JUN','JUL','AGO','SEP','OCT','NOV','DIC');		
	...
	$p1 = new LinePlot($datai);
	$graph->Add($p1);
	$p1->SetColor("#01DFD7");
	$p1->SetLegend('INGRESOS');
	
	$p2 = new LinePlot($datag);
	$graph->Add($p2);
	$p2->SetColor("#FF0000");
	$p2->SetLegend('GASTOS');

	$p3 = new LinePlot($datad);
	$graph->Add($p3);
	$p3->SetColor("#FE9A2E");
...
function info(){
	...
	global $mensaje;
	if($_SESSION['usuarios'] == $_SESSION['ref']){
		$mensaje = "\USUARIO ".$_SESSION['ref'].". ";}
	elseif($_SESSION['usuarios'] != $_SESSION['ref']){
	...
	global $text;
$text = "\n- BALANCE CONSULTA GRAFICA LINEAL ".$mensaje.$ActionTime.".\n\t* ".$titulo1.".\n\tORDENADA X\n\t".$coordenadax."\n\tDATOS INGRESOS\n\t".implode(", ",$datai)."\n\tDATOS GASTOS\n\t".implode(", ",$datag)."\n\tDATOS DIFERENCIALES\n\t".implode(", ",$datad);
